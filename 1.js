...
function do_duizhan1(renshu) {
    ...
    for (let x = 1; x <= 30;) {
        ...
        xuan_txt_list1 = allx_txt.match(/[a-d][^a-z\u4e00-\u9fa5\d]?\s*[^a-d\s]+(?=[a-d][^a-z\u4e00-\u9fa5\d]?|$)/gi);
        if (!xuan_txt_list) {
            log("识别不出选项");
            err_flag = false;
            sleep(200);
        ...
        log("原作者清洗后：" + xuan_txt_list.toString());//输出清洗后的选项列表
        log("修正版清洗后：" + xuan_txt_list1.toString());//输出清洗后的选项列表
